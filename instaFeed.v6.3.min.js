...
return this.each(function(){var data,opts,shortName,val;var container=$(this);var log=$.fn.instaFeed.log;if(container.data('instafeed-log')===false||(options&&options.log===false)||(opts&&opts.log===false)){}
...
return prop;}
return str;});},loadImages:function(container){var opts=this.opts();opts.API.log('requesting images from',opts.profile);$.ajax({url:opts.apiURL+opts.endpoint,dataType:'jsonp',type:'GET',data:{access_token:opts.token,count:opts.maxphotos},success:function(data){opts.API.log(opts);var $element=container,num=1,wrapcount=1,templateString,allowWrap,wrapperTmpString;if($element.length>0){for(var x in data.data){if(typeof data.data[x]==="object"){templateString=opts.API.template(opts.template,{parentCls:opts.parentclass,num:num++,lowResolutionURL:data.data[x].images.low_resolution.url},opts);allowWrap=opts.centerwrap===true&&opts.maxphotos%2!==0&&Math.round(opts.maxphotos /2);if(allowWrap===(num-1)){$element.append($(templateString).addClass('center-item')[0]);}else{$element.append(templateString);}}}
if(typeof opts.wrap=="number"&&opts.wrap>0){var wrapper=$element.find('.'+opts.parentclass).not('.center-item');for(var i=0;i<wrapper.length;i+=opts.wrap){wrapperTmpString=opts.API.template(opts.wrappertemplate,{wrapcount:wrapcount++},opts);wrapper.slice(i,i+opts.wrap).wrapAll(wrapperTmpString);}}}},error:function(data){console.log(data);}});}};$.fn.instaFeed.log=function log(){if(window.console&&console.log)
console.log('[instaFeed] '+Array.prototype.join.call(arguments,' '));};$.fn.instaFeed.defaults={apiURL:'https://api.instagram.com/v1',autoSelector:'.instafeed[data-instafeed-profile]',endPoint:'/users/self/media/recent',maxPhotos:20,wrap:0,wrapperTemplate:'<div class="wrap wrap_{{wrapcount}}"></div>',sync:true,log:true,parentClass:'insta',template:'<div class="{{parentCls}}" style="background-image:url({{lowResolutionURL}})"></div>',templateRegex:'{{((.)?.*?)}}',centerWrap:false};$(document).ready(function(){$($.fn.instaFeed.defaults.autoSelector).instaFeed();});})(jQuery);

...
return prop;}
return str;});},loadImages:function(container){var opts=this.opts();opts.API.log('requesting images from',opts.profile);$.ajax({url:opts.apiURL+opts.endpoint,dataType:'jsonp',type:'GET',data:{access_token:opts.token,count:opts.maxphotos},success:function(data){opts.API.log(opts);var $element=container,num=1,templateString,allowWrap;if($element.length>0){for(var x in data.data){if(typeof data.data[x]==="object"){templateString=opts.API.template(opts.template,{parentCls:opts.parentclass,num:num++,lowResolutionURL:data.data[x].images.low_resolution.url},opts);allowWrap=opts.centerwrap==true&&opts.maxphotos%2!==0&&Math.round(opts.maxphotos /2);if(allowWrap===(num-1)){$element.append($(templateString).addClass('center-item')[0]);}else{$element.append(templateString);}}}
